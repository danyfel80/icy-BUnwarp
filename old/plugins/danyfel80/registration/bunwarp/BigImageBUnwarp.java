...
public class BigImageBUnwarp extends BUnwarp {
	...
	// double[][] scales = { { 0.16 } };
	// EzVarDoubleArrayNative inUsedScales = new
	// EzVarDoubleArrayNative("Registration scales", scales, true);
	// - Subsampling factor
	...
	/*
	 * // - Landmark Weight EzVarDouble inLandmarkWeight = new
	 * EzVarDouble("Landmark Weight"); // - Image Weight
	 */
	EzVarDouble inImageWeight = new EzVarDouble("Image Weight");
	// - Consistency Weight
	EzVarDouble inConsistencyWeight = new EzVarDouble("Consistency Weight");

	EzGroup weightsGroup = new EzGroup("Weights", inDivWeight,
			inCurlWeight/* , inLandmarkWeight */, inImageWeight, inConsistencyWeight);
	...
	EzGroup advancedParamsGroup = new EzGroup("Advanced Parameters", inIniDef, inFnlDef, outputFileGroup, weightsGroup,
			inStopThreshold, inShowProcess);
	...
	protected void initialize() {
		...
		// inLandmarkWeight.setToolTipText("Weight related to landmarks present on
		// the sequence. Higher value means landmarks have more impact on the
		// result. Landmarks must be ROI2DPoints in the sequence.");
	...
	protected void execute() {
		...
		BigBUnwarpper bu = new BigBUnwarpper(srcPath, tgtPath, transformedSrcPath, transformedTgtPath, srcResultPath,
				...
				inSubsampleFactor.getValue(), inIniDef.getValue().getNumber(), inFnlDef.getValue().getNumber(),
				inDivWeight.getValue(), inCurlWeight.getValue(),
				0/* inLandmarkWeight.getValue() */, inImageWeight.getValue(), inConsistencyWeight.getValue(),
				inStopThreshold.getValue(), inShowProcess.getValue(), inMode.getValue().getNumber(),
