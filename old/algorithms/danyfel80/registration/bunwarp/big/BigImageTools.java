...
public class BigImageTools {
	...
	public static Dimension getSequenceSize(String path) {
		...
		try {
			...
			OMEXMLMetadata imgProps = (OMEXMLMetadata) importer.getOMEXMLMetaData();
	...
	public static void applyAndSaveTransformationToBigImage(String srcResultPath, String transformedSrcResultPath,
		...
		// Create full size result image
		LargeSequenceExporter exporter = new LargeSequenceExporter();
		exporter.setOutputFilePath(Paths.get(srcResultPath));
		OMEXMLMetadata resultMetadata = LargeSequenceExporter.createMetadata(resultTile.width, resultTile.height, srcChannels, srcDataType);
		exporter.setOutputImageMetadata(resultMetadata);
